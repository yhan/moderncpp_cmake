...
namespace basics {
    ...
    template<typename T>
    GArray<T>::GArray(std::initializer_list<T> init_list) : data(nullptr), size(static_cast<int>(init_list.size())) {
        if (size == 0) {
            capacity = 8;
            data = new T[capacity];
            return;
    ...
    void GArray<T>::resize_array() {
        if (capacity > 8 && size > 0 && size == capacity / 4) {
            // leave always doubled available spaces comparing to actual occupancy
            // Shrink the array
            capacity /= 2;
            T *new_data = new T[capacity];
            for (int i = 0; i < size; ++i) {
